...
class ManagerController extends AbstractController
{
    ...
    public function indexAction(Request $request, FileTypeService $fileTypeService)
    {
        ...
        /** @var Form $formMove */
        $folders = $this->getSubDirectories($fileManager->getBasePath(),$fileManager->getCurrentPath());
        $formMove = $this->createMoveForm($folders);
        ...
        $parameters['hasSubDirectories'] = !empty($this->getSubDirectories($fileManager->getBasePath(),$fileManager->getCurrentPath()));
    ...
    /**
     * Déplace plusieurs fichiers séléctionnés vers la destination séléctionnée.
     * @Route("/moveMuliple/", name="file_manager_move_multiple")
     *
     * @param $fileName
     *
     * @throws \Exception
     ...
     * @return \Symfony\Component\HttpFoundation\RedirectResponse
     */
    public function moveMultipleFileAction(Request $request)
    {
        $queryParameters = $request->query->all();
        $fileManager = $this->newFileManager($queryParameters);
        $folders = $this->getSubDirectories($fileManager->getBasePath(),$fileManager->getCurrentPath());
        $formMove = $this->createMoveForm($folders);
        /* @var Form $formMove */
        $formMove->handleRequest($request);
        if ($formMove->isSubmitted() && $formMove->isValid()) {
            $data = $formMove->getData();
            $fs = new Filesystem();
            if (isset($queryParameters['delete'])) {
                $isMoved = false;
                foreach ($queryParameters["delete"] as $file) {
                    $oldFilePath = realpath($fileManager->getCurrentPath() . DIRECTORY_SEPARATOR . $file);
                    $newFilePath = $data["folder"] . DIRECTORY_SEPARATOR . $file;
                    try {
                        $fs->rename($oldFilePath, $newFilePath);
                        $isMoved = true;
                    } catch (IOException $exception) {
                ...
                if ($isMoved) {
                    $this->addFlash('success', 'Le(s) fichier(s) ont été déplacé avec succès vers ' . $data["folder"]);
                }
            }
        }
        unset($queryParameters['delete']);
    ...
    /**
     * Déplace un fichier vers la destination séléctionnée
     * @Route("/move/{fileName}", name="file_manager_move")
     *
     * @param $fileName
     *
     * @throws \Exception
     ...
     * @return \Symfony\Component\HttpFoundation\RedirectResponse
     */
    public function moveFileAction(Request $request, $fileName)
    {
        $queryParameters = $request->query->all();
        $fileManager = $this->newFileManager($queryParameters);
        $folders = $this->getSubDirectories($fileManager->getBasePath(),$fileManager->getCurrentPath());
        ...
        if ($formMove->isSubmitted() && $formMove->isValid()) {
            $data = $formMove->getData();
            $oldFilePath = realpath($fileManager->getCurrentPath().DIRECTORY_SEPARATOR.$fileName);
            $newFilePath = $data["folder"].DIRECTORY_SEPARATOR.$fileName;
            $fs = new Filesystem();
            try {
                $fs->rename($oldFilePath, $newFilePath);
                $this->addFlash('success', 'Le fichier à été déplacé avec succès vers ' . $data["folder"]);
            } catch (IOException $exception) {
                $this->addFlash('danger', 'file.moved.danger');
            }
        }
        return $this->redirectToRoute('file_manager', $queryParameters);
    }
     ...
     * @return mixed
     */
    private function createMoveForm($subDirectories)
    {
        return $this->get('form.factory')->createNamedBuilder("rename")
    ...
    private function getSubDirectories($base_dir,$currentDir) {
        $key = substr($base_dir, strpos($base_dir, DIRECTORY_SEPARATOR, 10));
        $keyCurrent = substr($currentDir, strpos($currentDir, DIRECTORY_SEPARATOR, 10));
        $directories[$key] = $base_dir;
        foreach(scandir($base_dir) as $file) {
            if ($file == '.' || $file == '..') continue;
            $dir = $base_dir . DIRECTORY_SEPARATOR . $file;
            if (is_dir($dir)) {
                $key = substr($dir, strpos($dir, DIRECTORY_SEPARATOR, 10));
                $directories [$key] = $dir;
                $directories = array_merge($directories, $this->getSubDirectories($dir,$dir));
            }
        }
        unset($directories[$keyCurrent]);
        return $directories;
    }
}
